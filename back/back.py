...
class File(object):
        ...
        if self._from_root is None:
            ...
            if not self.pull:
                for root in self.mappings:
                    self.quietly("does %s start with %s ?" % (self.full_path, root))
                    if self.full_path.startswith(root):
                        roots.add(root)
                        self.quietly("found %s" % root)
                    self.quietly("does %s start with %s ?" % (self.path, root))
            ...
            if not self.push:
                for root, targets in self.mappings.items():
                    for target in targets:
                        self.quietly(
                            "does %s start with %s ?" % (self.full_path, target)
                        )
                        if self.full_path.startswith(target):
                            roots.add(root)
                            self.quietly("found root %s for target %s" % (root, target))
                        self.quietly("does %s start with %s ?" % (self.path, target))
    ...
    def is_from(self):
        ...
        self.quietly("does %s start with %s ?" % (self.full_path, self.from_root))
        return not self.pull and self.full_path.startswith(self.from_root)

    @property
    def is_to(self):
        """Is the file in a destination path."""
        self.quietly("is %s in any %s ?" % (self.full_path, self.to_roots))
        return not self.push and (
            any(self.path.startswith(root) for root in self.to_roots)
            or any(self.full_path.startswith(root) for root in self.to_roots)
        )
    ...
    def from_path(self):
        ...
        if self.is_from:
            ...
            to_root = [
                x
                for x in self.to_roots
                if self.path.startswith(x) or self.full_path.startswith(x)
            ][0]
            self.quietly(
                "replacing %s with %s in %s" % (to_root, self.from_root, self.full_path)
            )
    ...
    def from_path_remote(self):
        """Is the from path a remote (scp) path?"""
    ...
    def to_paths(self):
        ...
        return [
            self.from_path.replace(self.from_root, to_root) for to_root in self.to_roots
        ]
    ...
    def backup(self):
        ...
        self.quietly("from_path %s to_paths %s" % (self.from_path, self.to_paths))
        for to_path in self.to_paths:
